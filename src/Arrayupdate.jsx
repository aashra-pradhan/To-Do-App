  ...
  let original = people
      ...
      function handleCheck(nameToCheck) {
        const updatedPeople = people.map(person =>
          person.name === nameToCheck ? { ...person, checked: !person.checked } : person
        );
  ...
  const update = people.filter((p)=>p.name.toLowerCase().includes(search.toLowerCase()))
      return(
          <>
  ...
  <br />
  <button className="buttoncss" onClick={handleSearchClick}>Search</button>
      <br />
      <br />
      <br />
        ...
        (   <><input type="text" value={edit} onChange={handleEditChange} />
            <br />
            <button onClick={handleUpdate} className="buttoncss">Update</button>
            <br />
            
          </>
            )
            :
            (<>{person.checked?<span id="listitems"><del><em>{person.name}</em></del></span>:<span  id="listitems"><em>{person.name}</em></span>}
            
            <button className="buttoncss" id="options" onClick={() => handleDelete(person.name)}>Delete</button>
            <button className="buttoncss" id="options" onClick={() => handleCheck(person.name)}>Check</button>
            <button className="buttoncss" id="options" onClick={() => clickEdit(person.name)}>Edit</button>
            <br /></>)}
            </>
          ))
        }
